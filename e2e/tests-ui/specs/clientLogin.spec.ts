...
test.describe("Test Suite: Spanish Client LogIn - C&A Web", () => {
  test.beforeEach(async ({ page, handleCountryPopup, handleCookieBanner, setCookieConsent, context }) => {
    ...
    // // Setup the handler to dismiss the cookie banner automatically, added mid tests due to flakiness
  ...
  test("Test Case: Login step - missing email", { tag: '@unhappy-path', }, async ({ page, user }) => {
    ...
    await test.step('Client attept log in with missing email', async() => {
      await myAccount.login(MISSING_EMAIL, user.password); 
    });
  ...
  test("Test Case: Login step - invalid email format", {tag: '@unhappy-path', }, async ({ page, user }) => {
    const homePage = new HomePage(page);
    const myAccount = new MyAccountPage(page);
    ...
    await test.step('Client attept log in with wrong email', async() => {
      await myAccount.login(user.email + "@@@", user.password); 
    });
  ...
  test("Test Case: Login step - non Exsistent client", {tag: '@unhappy-path', }, async ({ page, user }) => {
    const homePage = new HomePage(page);
    const myAccount = new MyAccountPage(page);
    ...
    await test.step('Client attepts log in with non exsistent email', async() => {
      await myAccount.login("idontexsist@idontexsist.com", user.password); 
    });
  ...
  test("Test Case: Login step - invalid password", {tag: '@unhappy-path', }, async ({ page,user }) => {
    const homePage = new HomePage(page);
    const myAccount = new MyAccountPage(page);
    ...
    await test.step('Client attepts log in with non exsistent email', async() => {
      await myAccount.login(user.email, user.password + user.password); 
    });
  ...
  test("Test Case: Spanish client successful Login and log out", {tag: '@happy-path', }, async ({ page, user }) => {
    const homePage = new HomePage(page);
    const myAccount = new MyAccountPage(page);
